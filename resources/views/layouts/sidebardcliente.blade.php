            ...
            <div>
                ...
                <div class="h-20 px-6 flex items-center justify-between bg-slate-950/60 border-b border-slate-800">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-agro-700 to-agro-500 flex items-center justify-center text-white shadow-lg shadow-agro-900/40">
                ...
                <div class="px-4 py-6 space-y-6" :class="{ 'block': sidebarOpen, 'hidden lg:block': !sidebarOpen }">
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('profile.edit') ? 'bg-slate-800 text-agro-400 border-l-4 border-agro-500' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40' }}">
            ...
            <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-6 lg:px-10 flex items-center justify-between sticky top-0 z-30">
                ...
                <div class="flex items-center gap-4" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-100 transition-colors">
                        <span class="text-sm font-semibold text-slate-700">{{ Auth::user()->name ?? 'Cliente' }}</span>
