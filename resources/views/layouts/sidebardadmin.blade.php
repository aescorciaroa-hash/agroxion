        ...
        <aside class="w-full lg:w-72 bg-slate-900 text-slate-300 flex-shrink-0 flex flex-col justify-between border-r border-slate-800">
            <div>
                ...
                <div class="px-4 py-6 space-y-8 overflow-y-auto max-h-[calc(100vh-140px)]" :class="{ 'block': sidebarOpen, 'hidden lg:block': !sidebarOpen }">
                    ...
                    <!-- Dashboard -->
                    <div>
                        <a href="" class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-150 {{ request()->routeIs('dashboard') ? 'bg-agro-600 text-white shadow-md shadow-agro-900/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60' }}">
                    ...
                    <!-- GESTIÓN DE USUARIOS -->
                    <div class="space-y-1">
                        <p class="px-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Gestión de Usuarios</p>

                        <!-- Dropdown Gestión de Usuarios -->
                        <div x-data="{ open: true }" class="space-y-1">
                            <button @click="open = !open" type="button" 
                                    class="w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('usuarios.*') ? 'text-white bg-slate-800/80' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60' }} focus:outline-none">
                            ...
                            <div x-show="open" x-collapse x-cloak class="pl-9 pr-2 py-1 space-y-1">
                                <a href="{{ route('usuarios.create') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors {{ request()->routeIs('usuarios.create') ? 'text-agro-400 bg-slate-800 font-semibold' : 'text-slate-400 hover:text-white hover:bg-slate-800/50' }}">
                                    <i class="fas fa-user-plus text-[10px] text-agro-400"></i>
                                    <span>Registrar Usuario</span>
                                </a>
                                <a href="{{ route('usuarios.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors {{ request()->routeIs('usuarios.index') ? 'text-agro-400 bg-slate-800 font-semibold' : 'text-slate-400 hover:text-white hover:bg-slate-800/50' }}">
                    ...
                    <!-- CATÁLOGOS -->
                    <div class="space-y-1">
                        <p class="px-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Catálogos</p>

                        <a href="" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('proveedores.*') ? 'bg-slate-800 text-agro-400 border-l-4 border-agro-500' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40' }}">
                            <i class="fas fa-building w-5 text-center"></i>
                            <span>Proveedores</span>
                        </a>

                        <a href="" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('categorias.*') ? 'bg-slate-800 text-agro-400 border-l-4 border-agro-500' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40' }}">
                            <i class="fas fa-tags w-5 text-center"></i>
                            <span>Categorías</span>
                        </a>

                        <a href="" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('productos.*') ? 'bg-slate-800 text-agro-400 border-l-4 border-agro-500' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40' }}">
